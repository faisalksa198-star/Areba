...
interface MasterDataItem { id: string; name: string; is_active: boolean; }
...
const CATEGORY_TABS: { value: ProductCategory; label: string; icon: typeof Package }[] = [
  { value: 'kit', label: 'منتجات الأطقم', icon: Package },
  { value: 'scarf', label: 'الأوشحة', icon: Shirt },
  { value: 'hat', label: 'القبعات', icon: GraduationCap },
];
...
const emptyDefaults = () => ({
  abaya_design_id: '', sleeve_style_id: '', scarf_style_id: '', scarf_method_id: '',
  hat_style_id: '', font_id: '', date_type_id: '', embroidery_direction_id: '',
  embroidery_color: '', abaya_color: '', abaya_color_degree: '',
  scarf_color: '', scarf_color_degree: '', hat_color: '', hat_color_degree: '', sleeve_color: '',
});
...
export default function SallaProductsContent() {
  ...
  const [formCategory, setFormCategory] = useState<ProductCategory>('kit');
  const [formDefaults, setFormDefaults] = useState(emptyDefaults());
  ...
  const loadMasterData = useCallback(async () => {
    ...
    MASTER_TABLES.forEach((table, i) => { data[table] = (results[i].data as MasterDataItem[]) || []; });
    setMasterData(data);
  }, []);

  const loadProducts = useCallback(async () => {
    setLoading(true);
    const { data } = await supabase.from('salla_products').select('*').order('sort_order', { ascending: true });
    if (data) {
      const productIds = data.map(p => p.id);
      const { data: options } = await supabase
        .from('salla_product_options').select('*')
        .in('product_id', productIds.length > 0 ? productIds : ['none'])
        .order('sort_order', { ascending: true });
      const productsWithOptions = data.map(p => ({
        ...p,
        category: (p as any).category || 'kit',
        options: (options || []).filter(o => o.product_id === p.id).map(o => ({
          id: o.id, label: o.label, values: o.values || [], is_required: o.is_required,
          default_value: o.default_value || '', sort_order: o.sort_order || 0,
  ...
  useEffect(() => { loadProducts(); loadMasterData(); }, [loadProducts, loadMasterData]);

  // ===== Filtering =====
  const filtered = products.filter(p => {
    if (p.category !== activeTab) return false;
    if (search && !p.name.includes(search)) return false;
    return showArchived ? !p.is_active : p.is_active;
  });

  // ===== Form Helpers =====
  const resetFormDefaults = (product?: SallaProduct | null) => {
    if (!product) { setFormDefaults(emptyDefaults()); return; }
    setFormDefaults({
      abaya_design_id: product.abaya_design_id || '', sleeve_style_id: product.sleeve_style_id || '',
      scarf_style_id: product.scarf_style_id || '', scarf_method_id: product.scarf_method_id || '',
      hat_style_id: product.hat_style_id || '', font_id: product.font_id || '',
      date_type_id: product.date_type_id || '', embroidery_direction_id: product.embroidery_direction_id || '',
      embroidery_color: product.embroidery_color || '', abaya_color: product.abaya_color || '',
      abaya_color_degree: product.abaya_color_degree || '', scarf_color: product.scarf_color || '',
      scarf_color_degree: product.scarf_color_degree || '', hat_color: product.hat_color || '',
      hat_color_degree: product.hat_color_degree || '', sleeve_color: product.sleeve_color || '',
    });
  };

  const openCreate = () => {
    setEditingProduct(null);
    setFormName(''); setFormImage(null); setFormImagePreview('');
    setFormOptions([]); setFormCategory(activeTab); resetFormDefaults(null);
    setShowForm(true);
  };

  const openEdit = (product: SallaProduct) => {
    setEditingProduct(product);
    setFormName(product.name); setFormImage(null); setFormImagePreview(product.image_url || '');
    setFormOptions(product.options || []); setFormCategory(product.category); resetFormDefaults(product);
    setShowForm(true);
  };
  ...
  const duplicateProduct = (product: SallaProduct) => {
    ...
    setFormOptions((product.options || []).map(o => ({
      label: o.label, values: [...o.values], is_required: o.is_required,
      default_value: o.default_value, sort_order: o.sort_order, field_type: o.field_type,
    })));
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    setFormImage(file); setFormImagePreview(URL.createObjectURL(file));
  };
  ...
  // ===== Option Handlers =====
  const addOption = () => {
    setFormOptions(prev => [...prev, { label: '', values: [], is_required: false, default_value: '', sort_order: prev.length, field_type: 'dropdown' }]);
  };
  ...
  const addValueToOption = (index: number, value: string) => {
    ...
    setFormOptions(prev => prev.map((opt, i) => {
      if (i !== index || opt.values.includes(value.trim())) return opt;
      return { ...opt, values: [...opt.values, value.trim()] };
    }));
  };
  const removeValueFromOption = (optIndex: number, valIndex: number) => {
    setFormOptions(prev => prev.map((opt, i) => i !== optIndex ? opt : { ...opt, values: opt.values.filter((_, vi) => vi !== valIndex) }));
  };

  // ===== Save =====
  const handleSave = async () => {
    if (!formName.trim()) { toast({ title: 'يرجى إدخال اسم المنتج', variant: 'destructive' }); return; }
    setSaving(true);

    let imageUrl = editingProduct?.image_url || null;
    if (formImage) { imageUrl = await uploadImage(formImage); if (!imageUrl) { setSaving(false); return; } }

    // For kit category, include all defaults; for scarf/hat, nullify kit-specific fields
    const isKit = formCategory === 'kit';
    const productData: any = {
      name: formName.trim(), image_url: imageUrl, category: formCategory,
      abaya_design_id: isKit ? (formDefaults.abaya_design_id || null) : null,
      sleeve_style_id: isKit ? (formDefaults.sleeve_style_id || null) : null,
      scarf_style_id: isKit ? (formDefaults.scarf_style_id || null) : null,
      scarf_method_id: isKit ? (formDefaults.scarf_method_id || null) : null,
      hat_style_id: isKit ? (formDefaults.hat_style_id || null) : null,
      font_id: isKit ? (formDefaults.font_id || null) : null,
      date_type_id: isKit ? (formDefaults.date_type_id || null) : null,
      embroidery_direction_id: isKit ? (formDefaults.embroidery_direction_id || null) : null,
      embroidery_color: isKit ? (formDefaults.embroidery_color || null) : null,
      abaya_color: isKit ? (formDefaults.abaya_color || null) : null,
      abaya_color_degree: isKit ? (formDefaults.abaya_color_degree || null) : null,
      scarf_color: isKit ? (formDefaults.scarf_color || null) : null,
      scarf_color_degree: isKit ? (formDefaults.scarf_color_degree || null) : null,
      hat_color: isKit ? (formDefaults.hat_color || null) : null,
      hat_color_degree: isKit ? (formDefaults.hat_color_degree || null) : null,
      sleeve_color: isKit ? (formDefaults.sleeve_color || null) : null,
    };

    let productId: string;
    if (editingProduct) {
      const { error } = await supabase.from('salla_products').update(productData).eq('id', editingProduct.id);
      ...
      const { data, error } = await supabase.from('salla_products').insert(productData).select('id').single();
    ...
    if (validOptions.length > 0) {
      const optionsToInsert = validOptions.map((o, i) => ({
        product_id: productId, label: o.label.trim(),
        values: o.field_type === 'dropdown' ? o.values : [],
        is_required: o.is_required, default_value: o.default_value || null,
        sort_order: i, field_type: o.field_type,
      }));
      await supabase.from('salla_product_options').insert(optionsToInsert);
    }

    toast({ title: editingProduct ? 'تم التعديل ✓' : 'تمت الإضافة ✓' });
    setShowForm(false); loadProducts(); setSaving(false);
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    else { toast({ title: 'تم الحذف ✓' }); setProducts(prev => prev.filter(p => p.id !== id)); }
  };
  ...
  // ===== Render =====
  const categoryLabel = CATEGORY_TABS.find(c => c.value === activeTab)?.label || '';

  return (
    <>
      {/* Tabs */}
      <Tabs value={activeTab} onValueChange={v => { setActiveTab(v as ProductCategory); setSearch(''); setShowArchived(false); }} className="w-full">
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto" dir="rtl">
          <DialogHeader>
            <DialogTitle>{editingProduct ? `تعديل: ${editingProduct.name}` : `إضافة منتج جديد — ${categoryLabel}`}</DialogTitle>
          </DialogHeader>
          <div className="space-y-5 mt-2">
              ...
              <div>
                ...
                {formImagePreview ? (
                  <div className="relative w-full h-20 rounded-lg overflow-hidden border border-border">
                    ...
                    <button onClick={() => { setFormImage(null); setFormImagePreview(''); }} className="absolute top-1 left-1 bg-background/80 rounded-full p-1">
                      <X className="h-3 w-3" />
                    </button>
                  </div>
                ) : (
                    ...
                    {COLOR_FIELDS.map(field => (
                      <div key={field.key}>
                        ...
                        <Input
                          ...
                          placeholder={field.label} className="h-9 text-xs"
                        />
                      </div>
                    ))}
                  </div>
            ...
            {/* Dynamic Properties — available for all categories */}
                  ...
                  <CardContent className="p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <Select value={opt.field_type} onValueChange={v => updateOption(index, { field_type: v as FieldType, values: v !== 'dropdown' ? [] : opt.values, default_value: '' })}>
                      ...
                      <div>
                        ...
                        <Input value={opt.label} onChange={e => updateOption(index, { label: e.target.value })} placeholder="مثال: المقاس" className="text-sm h-9" />
                      </div>
                    </div>
                    ...
                    {opt.field_type === 'dropdown' && (
                      <div>
                        ...
                        <TagInput values={opt.values} onAdd={val => addValueToOption(index, val)} onRemove={valIdx => removeValueFromOption(index, valIdx)} placeholder="اكتب قيمة ثم اضغط Enter" />
                      </div>
                    )}
                    ...
                    {opt.field_type === 'text' && (
                      <div>
                        ...
                        <Input value={opt.default_value} onChange={e => updateOption(index, { default_value: e.target.value })} placeholder="القيمة الافتراضية (اختياري)" className="text-sm" />
                      </div>
                    )}

                    {opt.field_type === 'checkbox' && (
                      <div className="flex items-center gap-2">
                        <Checkbox checked={opt.default_value === 'true'} onCheckedChange={v => updateOption(index, { default_value: v ? 'true' : 'false' })} />
                    ...
                    {opt.field_type === 'dropdown' && opt.values.length > 0 && (
                      <div>
                        <label className="text-xs font-medium mb-1 block">القيمة الافتراضية</label>
                        <Select value={opt.default_value || '_none_'} onValueChange={v => updateOption(index, { default_value: v === '_none_' ? '' : v })}>
                          <SelectTrigger className="text-sm"><SelectValue placeholder="اختر القيمة الافتراضية" /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="_none_">-- بدون --</SelectItem>
                            {opt.values.map((val, vi) => (<SelectItem key={vi} value={val}>{val}</SelectItem>))}
                          </SelectContent>
                        </Select>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-2">
                      <Checkbox checked={opt.is_required} onCheckedChange={v => updateOption(index, { is_required: !!v })} />
...
// ===== Product Card =====
function ProductCard({ product, masterData, getMasterName, onEdit, onDuplicate, onDelete, onToggle, showKitDefaults }: {
  product: SallaProduct;
  masterData: Record<string, MasterDataItem[]>;
  getMasterName: (table: string, id: string | null) => string | null;
  onEdit: (p: SallaProduct) => void;
  onDuplicate: (p: SallaProduct) => void;
  onDelete: (id: string) => void;
  onToggle: (p: SallaProduct) => void;
  showKitDefaults: boolean;
}) {
  return (
    <Card className={`border-border/50 transition-opacity ${!product.is_active ? 'opacity-60' : ''}`}>
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          {product.image_url && (
            <img src={product.image_url} alt={product.name} className="w-16 h-16 rounded-lg object-cover border border-border/50 shrink-0" />
          )}
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <Switch checked={product.is_active} onCheckedChange={() => onToggle(product)} className="scale-75" />
              </div>
            </div>
            ...
            {showKitDefaults && (
              <div className="flex flex-wrap gap-1 mt-1">
                {DROPDOWN_FIELDS.map(field => {
                  ...
                  return name ? <Badge key={field.key} variant="secondary" className="text-[10px]">{field.label}: {name}</Badge> : null;
                })}
              </div>
            )}
            <div className="flex flex-wrap gap-1 mt-2">
              <Button variant="ghost" size="sm" onClick={() => onEdit(product)} className="h-7 px-2 text-xs gap-1"><Pencil className="h-3 w-3" />تعديل</Button>
              <Button variant="ghost" size="sm" onClick={() => onDuplicate(product)} className="h-7 px-2 text-xs gap-1"><Copy className="h-3 w-3" />تكرار</Button>
              <Button variant="ghost" size="sm" onClick={() => onDelete(product.id)} className="h-7 px-2 text-xs gap-1 text-destructive hover:text-destructive"><Trash2 className="h-3 w-3" />حذف</Button>
            </div>
          </div>
        </div>
      </CardContent>
...
function TagInput({ values, onAdd, onRemove, placeholder }: {
  values: string[]; onAdd: (val: string) => void; onRemove: (index: number) => void; placeholder: string;
}) {
  const [inputVal, setInputVal] = useState('');
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') { e.preventDefault(); if (inputVal.trim()) { onAdd(inputVal.trim()); setInputVal(''); } }
  };
  return (
    <div className="space-y-2">
      ...
      <div className="flex gap-2">
        <Input value={inputVal} onChange={e => setInputVal(e.target.value)} onKeyDown={handleKeyDown} placeholder={placeholder} className="text-sm flex-1" />
        <Button type="button" variant="outline" size="sm" onClick={() => { if (inputVal.trim()) { onAdd(inputVal.trim()); setInputVal(''); } }} disabled={!inputVal.trim()} className="shrink-0">
          <Plus className="h-3 w-3" />
        </Button>
      </div>
    </div>
