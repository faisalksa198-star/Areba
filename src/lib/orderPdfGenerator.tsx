...
import { ReportDocument, ReportData, ReportScarfDesign, ReportHatGroup, ReportStudentRow } from './pdf/ReportDocument';
...
export async function generateOrderPdf(orderId: string): Promise<void> {
  ...
  const logo = await toDataUrl(window.location.origin + '/logo.svg');

  // Preload scarf images
  const scarves: ReportScarfDesign[] = [];
  const scarfCodeMap = new Map<string, number>();
  for (let i = 0; i < scarfDesigns.length; i++) {
    const sd = scarfDesigns[i];
    ...
    scarves.push({
      ...
      styleImage: sd.scarf_styles?.image_url ? await toDataUrl(sd.scarf_styles.image_url) : '',
      dateImage: sd.date_types?.image_url ? await toDataUrl(sd.date_types.image_url) : '',
    });
  }
  ...
  const addToGroup = (id: string | null, name: string, fringe?: string) => {
    ...
    if (idx !== undefined) {
      ...
      hatGroups.push({ index: hatGroups.length + 1, name, image: img, count: 1, fringes: fringe ? [fringe] : [] });
    }
  };
  students.forEach((s: any) => addToGroup(s.hat_embroidery_id, s.hat_embroideries?.name || ''));
  extraHats.forEach((eh: any) => addToGroup(eh.hat_embroidery_id, eh.hat_embroideries?.name || '', eh.fringe_color));
