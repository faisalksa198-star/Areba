...
import {
  Plus, Pencil, Trash2, Loader2, ImagePlus, X, Search, Archive, ArchiveRestore,
} from 'lucide-react';
...
export default function SallaProducts() {
  ...
  const toggleArchive = async (product: SallaProduct) => {
    await supabase.from('salla_products').update({ is_active: !product.is_active }).eq('id', product.id);
    toast({ title: product.is_active ? 'تمت الأرشفة ✓' : 'تم إلغاء الأرشفة ✓' });
    loadProducts();
  };
  ...
  return (
    <DashboardLayout>
      <div className="space-y-5" dir="rtl">
        <div>
          <h1 className="text-2xl font-bold text-foreground">منتجات موقع سلة</h1>
          <p className="text-muted-foreground text-sm mt-1">إدارة كتالوج المنتجات المتوفرة لطلبات سلة</p>
        </div>
                    ...
                    <div className="flex items-start gap-2">
                      <div className="flex-1 space-y-2">
                        ...
                        <Input
                          ...
                          placeholder="القيم مفصولة بفواصل (مثال: 48, 50, 52, 54)"
                          className="text-sm"
                        />
                        <Input
